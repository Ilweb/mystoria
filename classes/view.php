...
class View
{
    ...
    {
		$variables["menu"] = new Menu($this->lang);
	...
	function thumbnail($image, $thmbValue = 200, $options = array())
	{
		...
		if ( strtolower($info['extension']) == 'jpg' )
		{
			...
			{
				$thmb_name = str_replace(".jpg", "w.jpg", $thmb_name);
				$thmb_name = str_replace(".JPG", "w.JPG", $thmb_name);
			}
			...
			if (!file_exists(IMAGE_DIR.$thmb_name))
			{
				...
				if ($options['crop'])
				{
					...
					if ($options['keepHigh'] && ($width / $height < $ratio) && ($width / $height < 1))
					{
						$ratio = $width / $height;
					}
					
					if ($options['keepWide'] && ($width / $height > $ratio) && ($width / $height > 1))
					{
						$ratio = $width / $height;
					}
				}
				...
				if (($thmbValue > 300) && ($options['watermark']))
				{
					...
					*/
					$stampWidth = round($new_width / 8);
					$stampHeight = round(($stampWidth / $sw) * $sh);
